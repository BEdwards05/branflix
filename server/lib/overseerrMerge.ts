...
const checkOverseerrMerge = async (): Promise<boolean> => {
  ...
  try {
    // Filter out the Jellyseerr migrations and replace them with the BranFlix migration
    ...
    const newMigrations: MixedList<string | Function> = migrations
      ...
      .map((migration) =>
        migration.name === 'UpdateWebPush1745492372230'
          ? BranFlixMigration1759769291608
  ...
  // Replace default Overseerr values with BranFlix values
  if (settings.main.applicationTitle === 'Overseerr') {
    settings.main.applicationTitle = 'BranFlix';
  }
  if (settings.notifications.agents.email.options.senderName === 'Overseerr') {
    settings.notifications.agents.email.options.senderName = 'BranFlix';
  }
  ...
  logger.info('Yeah! Overseerr to BranFlix migration completed successfully!', {
    label: 'BranFlix Migration',
  });
 ...
 */
class BranFlixMigration1759769291608 implements MigrationInterface {
  name = 'BranFlixMigration1759769291608';
