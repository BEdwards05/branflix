    ...
    if (!user && !(await userRepository.count())) {
      ...
      if (
        ...
        if (user) {
          ...
          logger.info(
            'Sign-in attempt from Plex user with access to the media server; creating new BranFlix user',
            {
              label: 'API',
              ip: req.ip,
              email: account.email,
...
authRoutes.post('/local', async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !(await user.passwordMatch(body.password))) {
      logger.warn('Failed sign-in attempt using invalid BranFlix password', {
        label: 'API',
        ip: req.ip,
        email: body.email,
        userId: user?.id,
    ...
    logger.error('Something went wrong authenticating with BranFlix password', {
      label: 'API',
      errorMessage: e.message,
      ip: req.ip,
      email: body.email,
